...
const Typewriter: React.FC<{ text: string }> = ({ text }) => {
  ...
  // Si el usuario prefiere movimiento reducido, desactivar animación
  const shouldAnimate = isTypewriterEnabled && !prefersReducedMotion;

  useEffect(() => {
    // Si la animación está desactivada (configuración o preferencia del sistema), mostrar todo inmediatamente
    if (!shouldAnimate) {
...
const CaseScreen: React.FC<CaseScreenProps> = ({ currentCase, onAssess }) => {
  const [userGCS, setUserGCS] = useState<GCSScore>({
    ocular: null,
    verbal: null,
    motor: null,
  });
  ...
  return (
    <div className="min-h-full paper-texture-overlay relative">
      ...
      {showWarning && !isComplete && (
        <p className="text-center text-red-600 dark:text-red-400 mt-6 text-sm animate-pulse">
          Por favor, completa la valoración de las tres respuestas.
        </p>
      )}
      ...
      <div className="flex justify-center mt-6">
        <button
          ...
          disabled={!isComplete}
          className={
            "flex flex-row items-center gap-3 aspect-[4/1] px-6 py-3 select-none text-[20px] sm:text-[25px] font-league-gothic tracking-wider rounded-xl " +
