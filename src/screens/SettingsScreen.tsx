...
const ToggleSwitch: React.FC<{
  ...
  return (
    <div className="flex items-center justify-between p-4 bg-stone-200/50 dark:bg-stone-700/50 rounded-lg border border-stone-300 dark:border-stone-600 shadow-sm">
      <span className="font-lora text-stone-800 dark:text-stone-200 text-lg">
        {label}
      </span>
      <button
        onClick={onChange}
  ...
  return (
    <div className="flex items-center justify-between p-4 bg-stone-200/50 dark:bg-stone-700/50 rounded-lg border border-stone-300 dark:border-stone-600 shadow-sm">
      ...
      >
        ...
        <option value="background1">Operativo EMS</option>
      </select>
    </div>
  );
};
...
const SettingsScreen: React.FC = () => {
  const { isMuted, theme, isTypewriterEnabled, appBackground } = useAppState();
    ...
    <div className="space-y-6">
      <div className="space-y-1">
        ...
        <ToggleSwitch
          ...
          IconOn={
            <span className="font-typewriter text-xs font-bold text-amber-600">
              Tt
            </span>
          }
          IconOff={
            <span className="font-sans text-xs font-bold text-stone-500">
              T
            </span>
          }
        />
      ...
      <div className="p-1 bg-stone-100/50 dark:bg-stone-800/50 rounded-lg text-center text-sm text-stone-500 dark:text-stone-400 border border-stone-200 dark:border-stone-700">
        <p className="font-league-gothic tracking-wider text-lg mb-1">
          Simulador de Casos Glasgow
        </p>
        <p>v2.0.0</p>
        <p className="mt-2 italic font-serif">
          "Nunca confíes en las impresiones generales, muchacho, concéntrate en
          los detalles."
        </p>
      </div>
    </div>
