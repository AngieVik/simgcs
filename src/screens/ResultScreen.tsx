...
const ScoreDisplay: React.FC<{
  ...
  return (
    <div className="text-center flex flex-col items-center justify-start p-2">
      <p className="font-lora font-bold text-xl text-stone-800 dark:text-stone-300 mb-2">
        {label}
      </p>
      <div
        className={`grid grid-cols-[24px,1fr,24px] items-center justify-center gap-1 w-full text-center ${
          isCorrect ? "text-amber-600 dark:text-amber-300" : "player-score-text"
        }`}
      >
...
const actionButtonBase =
  "flex items-center justify-center gap-2 px-2 py-2 rounded-lg text-xl font-league-gothic tracking-wider transition-all duration-200 transform hover:scale-[1.03] active:scale-[0.98] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-400";
...
const ResultScreen: React.FC<ResultScreenProps> = ({
  ...
  useEffect(() => {
    setQuote(
      getRandomQuote(lastCase.isCorrect ? SUCCESS_QUOTES : FAILURE_QUOTES)
    );
  ...
  return (
    <div>
      <p className="text-lg italic my-4 text-center text-stone-600 dark:text-stone-400">
        "{quote}"
      </p>
      ...
      <div className="relative p-4 my-6 bg-stone-100/50 dark:bg-stone-900/20 rounded-lg border border-stone-300 dark:border-stone-700 shadow-inner shadow-lg shadow-black/10">
        <div
          className={`verdict-stamp ${isStamped ? "stamped" : ""} ${
            lastCase.isCorrect
              ? "text-green-800 border-green-800 dark:text-green-400 dark:border-green-400"
              : "text-rose-900 border-rose-900 dark:text-rose-400 dark:border-rose-400"
          } z-10`}
        >
          {lastCase.isCorrect ? "CORRECTO" : "INCORRECTO"}
        </div>
        <h4 className="font-lora font-bold text-lg text-stone-800 dark:text-stone-300 mb-3 text-center">
          Informe de Valoración
        </h4>
        <div className="grid grid-cols-4 items-start text-center divide-x divide-stone-300 dark:divide-stone-700">
          <ScoreDisplay
            userScore={lastCase.userGCS.ocular}
            correctScore={lastCase.correctGCS.ocular}
            label="O"
          />
          <ScoreDisplay
            userScore={lastCase.userGCS.verbal}
            correctScore={lastCase.correctGCS.verbal}
            label="V"
          />
          <ScoreDisplay
            userScore={lastCase.userGCS.motor}
            correctScore={lastCase.correctGCS.motor}
            label="M"
          />
          <ScoreDisplay
            userScore={userTotal}
            correctScore={correctTotal}
            label="T"
          />
        </div>
      </div>
      ...
      <div className="mt-6 space-y-4">
        <details className="bg-stone-200/50 dark:bg-stone-900/30 rounded-lg border border-stone-300 dark:border-stone-700 transition-all duration-300 open:bg-stone-200 dark:open:bg-stone-900/50 open:shadow-lg shadow-md shadow-black/5">
          <summary className="font-bold text-lg text-amber-700 dark:text-amber-400 p-4">
            Lectura del caso
          </summary>
          <div className="px-4 pb-4 border-t border-stone-300 dark:border-stone-700">
            <p className="text-stone-700 dark:text-stone-300 whitespace-pre-wrap leading-relaxed mt-2">
              {lastCase.narrative}
            </p>
          </div>
        </details>
        <div className="p-4 bg-transparent rounded-lg border border-stone-300 dark:border-stone-700 shadow-lg shadow-black/10">
          <h4 className="font-bold text-lg text-amber-700 dark:text-amber-400 mb-2">
            Conclusión del Caso
          </h4>
      ...
      <div className="flex gap-3 w-full mt-2">
        <button onClick={onActivarnos} className={primaryButton}>
          <AmbulanceIcon className="w-7 h-7" />
          Siguiente caso
        </button>
        <button onClick={onEndService} className={secondaryButton}>
